...
class DistributedSeedingStrategy:
    ...
    def get_indices(self, seed_indices: Iterator[int], n_islands: int) -> dict[int, list[int]]:
    ...
    def get_island_seed_map(self) -> dict[int, list[int]]:
        ...
        match self.seed_island_strategy:
            case SeedIslandStrategy.CONCENTRATED:
                return ConcentratedSeedingStrategy().get_indices(self.iter_seeds(), self.n_islands, self.pop_size)
            case SeedIslandStrategy.DISTRIBUTED:
                return DistributedSeedingStrategy().get_indices(self.iter_seeds(), self.n_islands)
            case _:
                return DistributedSeedingStrategy().get_indices(self.iter_seeds(), self.n_islands)
