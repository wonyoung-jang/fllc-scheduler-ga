...
class LocationWeightsModel(BaseModel):
    ...
    def validate(self) -> LocationWeightsModel:
        ...
        if self.inter_rounds + self.intra_rounds <= 0.0:
...
def build_app_config_model(path: Path = CONFIG_FILE_DEFAULT) -> AppConfigModel:
    """Create and return the application configuration."""
    if not path.exists():
        msg = f"Configuration file does not exist at: {path}"
        raise FileNotFoundError(msg)
    return AppConfigModel.model_validate_json(path.read_text())
